...
//bool CSentinelHacker::SeqSolve (const vector<uint64_t> &fragments, CBigInt &cnt) {

//}


//void CSentinelHacker::AddReceiver (AReceiver recv) {

//}

//void CSentinelHacker::AddTransmitter (ATransmitter trans) {
...
//the method starts all required thread (receivers, transmitters and workers) and starts to process the incoming message fragments. The parameter is an integer thrCnt,
// it defines the number of worker threads to start. Once the threads are started, the method returns to the caller. It does not wait for the completion of the threads.
//void CSentinelHacker::Start(unsigned thrCnt) {
  //  cout << thrCnt;
//}
